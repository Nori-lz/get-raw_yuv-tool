...
def get_raw_yuv(hostname,savepath):
    # 创建保存目录（新增）
    ...
    # 在删除目录后增加等待时间
    time.sleep(1)  # 新增等待1秒确保目录删除完成
    httpSet(hostname,'"sec":6;"cmd":112;"page":3')  #page=3：抓取raw图数量设置
    httpSet(hostname,'"sec":6;"cmd":113;"page":3')  #page=3：抓取yuv图数量设置
    stdoutraw, stderr = exec_command_and_wait(ssh,'ls /nextvpu/example/media/vi_venc_rtsp/data/raw')
    raw_files = [f for f in stdoutraw.strip().split() if f]
    print(raw_files)
    stdoutyuv, stderr = exec_command_and_wait(ssh,'ls /nextvpu/example/media/vi_venc_rtsp/data/yuv')
    yuv_files = [f for f in stdoutyuv.strip().split() if f]
    print(yuv_files)
    for i,raw_file in enumerate(raw_files):
        ...
        if raw_file:
            # 修改文件名处理：去除时间戳部分
            raw_base = re.sub(r'_\d+s_\d+ms\.raw$', '', raw_file)
            rawsavepath = f'{savepath}/{raw_base}_exp{SYSTEM_INTEGRATION_TIME}_gain{SYSTEM_SENSOR_ANALOG_GAIN}_r{SYSTEM_AWB_RED_GAIN}_b{SYSTEM_AWB_BLUE_GAIN}{spectrum}{sequence}.raw'
            try:
                sftp.get(f'/nextvpu/example/media/vi_venc_rtsp/data/raw/{raw_file}', rawsavepath)
                print(f'RAW文件已保存：{rawsavepath}')
            except Exception as e:
                print(f'RAW文件传输失败：{str(e)}')
    ...
    for i,yuv_file in enumerate(yuv_files):
        ...
        if yuv_file:
            # 修改文件名处理：去除时间戳部分
            yuv_base = re.sub(r'_\d+s_\d+ms\.yuv$', '', yuv_file)
            yuvsavepath = f'{savepath}/{yuv_base}_exp{SYSTEM_INTEGRATION_TIME}_gain{SYSTEM_SENSOR_ANALOG_GAIN}_r{SYSTEM_AWB_RED_GAIN}_b{SYSTEM_AWB_BLUE_GAIN}{spectrum}{sequence}.yuv'
            try:
                sftp.get(f'/nextvpu/example/media/vi_venc_rtsp/data/yuv/{yuv_file}', yuvsavepath)
                print(f'YUV文件已保存：{yuvsavepath}')
                ...
                bgrsavepath = f'{savepath}/{yuv_base}_exp{SYSTEM_INTEGRATION_TIME}_gain{SYSTEM_SENSOR_ANALOG_GAIN}_r{SYSTEM_AWB_RED_GAIN}_b{SYSTEM_AWB_BLUE_GAIN}{spectrum}{sequence}.png'
                cv2.imwrite(bgrsavepath, bgr)
                print(bgrsavepath)
            except Exception as e:
                print(f'YUV文件传输失败：{str(e)}')
            # 关闭 SFTP 会话，释放系统资源
    sftp.close()
